...
void interrupt low_priority SlowTick() {
    ...
    signed int VoltageRL = 255;
    signed int CurrentRL = 0;

    //---------------------------------------------------------------------------
    //Figure out the Power Scaling...
    //---------------------------------------------------------------------------
    VoltageRL=255+(internal_BatteryVoltage-Config.VoltageLimit_counts);
    if (VoltageRL<0) VoltageRL=0;
    if (VoltageRL>255) VoltageRL=255;

    CurrentRL=255+(Config.CurrentLimit_counts-internal_MotorCurrent);
    if (CurrentRL<0) CurrentRL=0;
    if (CurrentRL>255) CurrentRL=255;

    if (CurrentRL<VoltageRL) PID_ResponseLimit=CurrentRL; else PID_ResponseLimit=VoltageRL;
    PID_ResponseLimit<<=2;
